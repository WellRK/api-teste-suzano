...
export class TipoTransporteController {
  private readonly _logger = new Logger(TipoTransporteController.name);

  constructor(private readonly _service: TipoTransporteService) {}

  @Post()
  @HttpCode(201)
  async create(@Body() dto: CreateTipoTransporteDto) {
    try {
  ...
  @Patch(':id')
  @HttpCode(200)
  async update(@Param('id') id: string, @Body() dto: UpdateTipoTransporteDto) {
    try {
